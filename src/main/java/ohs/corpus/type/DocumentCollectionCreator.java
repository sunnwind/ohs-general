...
public class DocumentCollectionCreator {
	...
	public static void main(String[] args) throws Exception {
		...
		DocumentCollectionCreator ldcc = new DocumentCollectionCreator();
		ldcc.setCreateVocabBatchSize(200);
		ldcc.setCountingThreadSize(10);
		ldcc.setIndexingThreadSize(5);
		// ldcc.setReuseVocab(true);
		// ldcc.setEncode(true);

		// ldcc.create(MIRPath.OHSUMED_COL_DC_DIR, 1, new int[] { 3, 5 });
		// ldcc.create(MIRPath.TREC_CDS_2014_COL_DC_DIR, 0, new int[] { 1, 2, 3 });
		// ldcc.create(MIRPath.TREC_CDS_2016_COL_DC_DIR, 0, new int[] { 1, 2, 3 });
		// ldcc.create(MIRPath.BIOASQ_COL_DC_DIR, 0, new int[] { 4, 5 });
		// ldcc.create(MIRPath.WIKI_COL_DC_DIR, 0, new int[] { 3 });
		// ldcc.create(MIRPath.CLUEWEB_COL_DC_DIR, 0, new int[] { 1 });

		// ldcc.create(MIRPath.CLEF_EH_2014_COL_DC_DIR, 0, new int[] { 3 });
		// ldcc.create(MIRPath.TREC_GENO_2007_COL_DC_DIR, 0, new int[] { 1 });
		// ldcc.create(MIRPath.CLUEWEB_COL_DC_DIR, 0, new int[] { 1 });
		...
		// ldcc.setStringNormalizer(new ThreePStringNormalizer());
		// ldcc.create(KPPath.COL_DC_DIR, 0, new int[] { 4, 5, 6, 7 });

		// InvertedIndexCreator.main(args);

		ldcc.create("../../data/naver_news/col/dc/", 0, new int[] { 0, 2 });
	...
	private Vocab createVocab(Counter<String> docFreqs, Counter<String> wordCnts) {
		...
		for (String word : docFreqs.keySet()) {
			...
			if (word.length() > max_word_len || doc_freq < min_word_cnt) {
