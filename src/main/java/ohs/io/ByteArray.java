...
public class ByteArray implements RandomAccess, Cloneable, java.io.Serializable, Iterable<Byte> {
	/** Index-based split-by-two, lazily initialized Spliterator */
	static final class ArrayListSpliterator<Byte> implements Spliterator<Byte> {
		...
		ArrayListSpliterator(ByteArray list, int origin, int fence, int expectedModCount) {
		...
		public ArrayListSpliterator<Byte> trySplit() {
			int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
			return (lo >= mid) ? null : // divide range in half unless too small
					new ArrayListSpliterator<Byte>(list, lo, index = mid, expectedModCount);
		}
	}
	...
	private class Itr implements Iterator<Byte> {
		...
		public void forEachRemaining(Consumer consumer) {
			...
			final int size = ByteArray.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final byte[] elementData = ByteArray.this.vals;
		...
		public Byte next() {
			...
			byte[] elementData = ByteArray.this.vals;
			if (i >= elementData.length)
				throw new ConcurrentModificationException();
			cursor = i + 1;
	 ...
	 * added.
	 */
	transient byte[] vals; // non-private to simplify nested class access
	 ...
	 */
	public ByteArray(Collection<Byte> a) {
		vals = new byte[a.size()];
		size = vals.length;
	...
	public ByteArray(int initialCapacity) {
		if (initialCapacity > 0) {
			this.vals = new byte[initialCapacity];
	...
	public ByteArray(ObjectInputStream ois) throws Exception {
		readObject(ois);
	}
	...
	public boolean addAll(ByteArray c) {
		int numNew = c.size();
		ensureCapacityInternal(size + numNew); // Increments modCount
		for (byte v : c) {
			vals[size++] = v;
	...
	private boolean batchRemove(Collection<Byte> c, boolean complement) {
		final byte[] elementData = this.vals;
		int r = 0, w = 0;
	...
	public ByteSize byteSize() {
		return new ByteSize(Byte.BYTES * size());
	}
	...
	public ByteArray get(int[] locs) {
		ByteArray ret = new ByteArray(locs.length);
		for (int loc : locs) {
			ret.add(get(loc));
		}
		return ret;
	...
	public boolean removeIf(Predicate<Byte> filter) {
