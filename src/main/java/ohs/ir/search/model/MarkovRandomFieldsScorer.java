...
public class MarkovRandomFieldsScorer extends LanguageModelScorer {
	...
	public MarkovRandomFieldsScorer(DocumentSearcher ds) {
		this(ds.getVocab(), ds.getDocumentCollection(), ds.getInvertedIndex());
	}

	public MarkovRandomFieldsScorer(Vocab vocab, DocumentCollection dc, InvertedIndex ii) {
		super(vocab, dc, ii);
		setType(Type.QL);
	}
	...
	public SparseVector scoreFromCollection(SparseVector Q, SparseVector docs) throws Exception {
		SparseVector s1 = mixtures.value(0) > 0 ? super.scoreFromCollection(Q, docs)
				: new SparseVector(ArrayUtils.copy(docs.indexes()));
		SparseVector s2 = mixtures.value(1) > 0 ? scoreOrderedPhrases(Q, docs)
				: new SparseVector(ArrayUtils.copy(docs.indexes()));
		SparseVector s3 = mixtures.value(2) > 0 ? scoreUnorderedPhrases(Q, docs)
				: new SparseVector(ArrayUtils.copy(docs.indexes()));
	...
	public SparseVector scoreFromIndex(SparseVector Q, SparseVector docs) throws Exception {
		SparseVector s1 = mixtures.value(0) > 0 ? super.scoreFromIndex(Q, docs)
				: new SparseVector(ArrayUtils.copy(docs.indexes()));
		SparseVector s2 = mixtures.value(1) > 0 ? scoreOrderedPhrases(Q, docs)
				: new SparseVector(ArrayUtils.copy(docs.indexes()));
		SparseVector s3 = mixtures.value(2) > 0 ? scoreUnorderedPhrases(Q, docs)
	...
	public SparseVector scorePhrases(SparseVector Q, SparseVector docs, boolean keep_order, int window_size)
			throws Exception {
