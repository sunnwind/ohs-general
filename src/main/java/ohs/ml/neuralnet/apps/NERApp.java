...
public class NERApp {
	public static TextFeatureExtractor getNERFeatureExtractor(String extFileName, LDocument C) throws Exception {
		...
		if (extFileName != null) {
			...
			Set<String> wikis = Generics.newHashSet();
	...
	public static void run() throws Exception {
		...
		transformBIOtoBIOSE(C1);
		transformBIOtoBIOSE(C2);
...

		{
			...
			Set<String> labels = Generics.newTreeSet(C.getTokenStrings(3));
			labels.remove("O");
		...
		NeuralNet nn = new NeuralNet(labelIdxer, new Vocab(wVocab), TaskType.TOKEN_SEQ_LABELING);
		...
		if (read_ner_model && FileUtils.exists(modelFileName)) {
			...
			nn.add(new DropoutLayer());
			nn.add(new FullyConnectedLayer(l1_size * 2, label_size));
