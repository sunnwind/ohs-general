...
public class DoubleArray implements RandomAccess, Cloneable, java.io.Serializable, Iterable<Double> {
	...
	static final class ArrayListSpliterator<Double> implements Spliterator<Double> {
		...
		private final DoubleArray list;

		/** Create new spliterator covering the given range */
		ArrayListSpliterator(DoubleArray list, int origin, int fence, int expectedModCount) {
		...
		public ArrayListSpliterator<Double> trySplit() {
			int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
			return (lo >= mid) ? null : // divide range in half unless too small
					new ArrayListSpliterator<Double>(list, lo, index = mid, expectedModCount);
		}
	}
	...
	private class Itr implements Iterator<Double> {
		...
		public void forEachRemaining(Consumer consumer) {
			...
			final int size = DoubleArray.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final double[] elementData = DoubleArray.this.vals;
		...
		public Double next() {
			...
			double[] elementData = DoubleArray.this.vals;
			if (i >= elementData.length)
				throw new ConcurrentModificationException();
			cursor = i + 1;
		...
		public void remove() {
			...
			try {
				DoubleArray.this.remove(lastRet);
				cursor = lastRet;
				lastRet = -1;
	...
	private class ListItr extends Itr implements ListIterator<Double> {
		...
		public void set(Double e) {
			...
			try {
				DoubleArray.this.set(lastRet, e);
			} catch (IndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		}
	 ...
	 * buffer. Any empty ArrayList with vals == DEFAULTCAPACITY_EMPTY_VALUES will be expanded to DEFAULT_CAPACITY when the first element is
	 * added.
	 */
	transient double[] vals; // non-private to simplify nested class access

	/**
	 * Constructs an empty list with an initial capacity of ten.
	 */
	public DoubleArray() {
		this.vals = DEFAULTCAPACITY_EMPTY_VALUES;
	}
	...
	public DoubleArray(Collection<Double> c) {
		vals = new double[c.size()];
		for (double v : c) {
			vals[size++] = v;
		}
	}

	public DoubleArray(double[] elementData) {
	...
	public DoubleArray(int initialCapacity) {
		if (initialCapacity > 0) {
			this.vals = new double[initialCapacity];
	...
	public DoubleArray(ObjectInputStream ois) throws Exception {
		readObject(ois);
	}
	...
	private boolean batchRemove(Collection<Double> c, boolean complement) {
		final double[] elementData = this.vals;
		int r = 0, w = 0;
	...
	public void ensureCapacity(int minCapacity) {
		int minExpand = (vals != DEFAULTCAPACITY_EMPTY_VALUES)
	...
	private void ensureCapacityInternal(int minCapacity) {
		if (vals == DEFAULTCAPACITY_EMPTY_VALUES) {
	...
	public DoubleArray get(int[] locs) {
		DoubleArray ret = new DoubleArray(locs.length);
		for (int loc : locs) {
			ret.add(get(loc));
		}
		return ret;
	...
	public void readObject(ObjectInputStream ois) throws Exception {
		vals = DEFAULTCAPACITY_EMPTY_VALUES;
	...
	public boolean removeIf(Predicate<Double> filter) {
	...
	public DoubleArray subArray(int fromIndex, int toIndex) {
		...
		DoubleArray ret = new DoubleArray(size);
