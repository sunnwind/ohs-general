...
public class IntegerArray implements RandomAccess, Cloneable, java.io.Serializable, Iterable<Integer> {
	...
	static final class ArrayListSpliterator<Integer> implements Spliterator<Integer> {
		...
		private final IntegerArray list;
		private int index; // current index, modified on advance/split
		private int fence; // -1 until used; then one past last index
		private int expectedModCount; // initialized when fence set

		/** Create new spliterator covering the given range */
		ArrayListSpliterator(IntegerArray list, int origin, int fence, int expectedModCount) {
		...
		public ArrayListSpliterator<Integer> trySplit() {
			int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
			return (lo >= mid) ? null : // divide range in half unless too small
					new ArrayListSpliterator<Integer>(list, lo, index = mid, expectedModCount);
		}
	}
	...
	private class Itr implements Iterator<Integer> {
		...
		public void forEachRemaining(Consumer consumer) {
			...
			final int size = IntegerArray.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final int[] elementData = IntegerArray.this.vals;
		...
		public Integer next() {
			...
			int[] elementData = IntegerArray.this.vals;
			if (i >= elementData.length)
				throw new ConcurrentModificationException();
			cursor = i + 1;
		...
		public void remove() {
			...
			try {
				IntegerArray.this.remove(lastRet);
				cursor = lastRet;
				lastRet = -1;
	...
	private class ListItr extends Itr implements ListIterator<Integer> {
		...
		public void set(Integer e) {
			...
			try {
				IntegerArray.this.set(lastRet, e);
			} catch (IndexOutOfBoundsException ex) {
				throw new ConcurrentModificationException();
			}
		}
	 ...
	 */
	public IntegerArray(Collection<Integer> a) {
		vals = new int[a.size()];
		int i = 0;
		for (int b : a) {
			vals[i++] = b;
	...
	public IntegerArray(int initialCapacity) {
		if (initialCapacity > 0) {
			this.vals = new int[initialCapacity];
	...
	public IntegerArray(String fileName) throws Exception {
	...
	public boolean addAll(IntegerArray c) {
		int numNew = c.size();
		ensureCapacityInternal(size + numNew); // Increments modCount
		for (int v : c) {
			vals[size++] = v;
	...
	private boolean batchRemove(Collection<Integer> c, boolean complement) {
		final int[] elementData = this.vals;
		int r = 0, w = 0;
	...
	public ByteSize byteSize() {
		return new ByteSize(Integer.BYTES * size());
	}
	...
	public IntegerArray clone() {
		try {
			IntegerArray v = (IntegerArray) super.clone();
	...
	public IntegerArray get(int[] locs) {
		IntegerArray ret = new IntegerArray(locs.length);
		for (int loc : locs) {
			ret.add(get(loc));
		}
		return ret;
	...
	public IntegerArray subArray(int[] is) {
		IntegerArray ret = new IntegerArray(is.length);
		for (int i : is) {
			ret.add(get(i));
		}
		return ret;
