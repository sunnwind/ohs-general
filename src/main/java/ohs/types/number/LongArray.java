...
public class LongArray implements RandomAccess, Cloneable, java.io.Serializable, Iterable<Long> {
	...
	static final class ArrayListSpliterator<Long> implements Spliterator<Long> {
		...
		public ArrayListSpliterator<Long> trySplit() {
			int hi = getFence(), lo = index, mid = (lo + hi) >>> 1;
			return (lo >= mid) ? null : // divide range in half unless too small
					new ArrayListSpliterator<Long>(list, lo, index = mid, expectedModCount);
		}
	}
	...
	private class Itr implements Iterator<Long> {
		...
		public void forEachRemaining(Consumer consumer) {
			...
			final int size = LongArray.this.size;
			int i = cursor;
			if (i >= size) {
				return;
			}
			final long[] elementData = LongArray.this.vals;
		...
		public Long next() {
			...
			long[] elementData = LongArray.this.vals;
			if (i >= elementData.length)
				throw new ConcurrentModificationException();
			cursor = i + 1;
	 ...
	 */
	public LongArray() {
		this.vals = DEFAULTCAPACITY_EMPTY_ELEMENTDATA;
	}
	...
	public LongArray(int initialCapacity) {
		if (initialCapacity > 0) {
			this.vals = new long[initialCapacity];
		} else if (initialCapacity == 0) {
			this.vals = EMPTY_ELEMENTDATA;
		} else {
	...
	public LongArray(long[] elementData) {
		this.vals = elementData;
		size = elementData.length;
	}

	public LongArray(ObjectInputStream ois) throws Exception {
		readObject(ois);
	}
	...
	public boolean addAll(LongArray c) {
		int numNew = c.size();
		ensureCapacityInternal(size + numNew); // Increments modCount
		for (long v : c) {
			vals[size++] = v;
	...
	private boolean batchRemove(Collection<Long> c, boolean complement) {
		final long[] elementData = this.vals;
		int r = 0, w = 0;
	...
	public ByteSize byteSize() {
		return new ByteSize(Long.BYTES * size());
	}
	...
	public LongArray clone() {
		try {
			LongArray v = (LongArray) super.clone();
	...
	public void ensureCapacity(int minCapacity) {
		int minExpand = (vals != DEFAULTCAPACITY_EMPTY_ELEMENTDATA)
	...
	private void ensureCapacityInternal(int minCapacity) {
		if (vals == DEFAULTCAPACITY_EMPTY_ELEMENTDATA) {
	...
	public LongArray get(int[] locs) {
		LongArray ret = new LongArray(locs.length);
		for (int loc : locs) {
			ret.add(get(loc));
		}
		return ret;
	...
	public void readObject(ObjectInputStream ois) throws Exception {
		vals = DEFAULTCAPACITY_EMPTY_ELEMENTDATA;
	...
	public boolean removeIf(Predicate<Long> filter) {
	...
	public void trimToSize() {
		modCount++;
		if (size < vals.length) {
			vals = (size == 0) ? EMPTY_ELEMENTDATA : Arrays.copyOf(vals, size);
		}
	}
